...
def detectPartFromImageAndSave(partimage, x, y, w, h):
	...
	if not cv2.imwrite(filename,partimage):
		...
		image_id = InsertIntoPartimages(filename, x, y, w, h, ts)
...
def InsertIntoPartimages(filename, x, y, w, h, ts):
	global connection, cursor, args
	size = os.path.getsize(filename)
	run_id = int(args["run_id"])
	imported = ts.strftime('%Y-%m-%d %H:%M:%S')
	created = time.strftime('%Y-%m-%d %H:%M:%S')
	sql = "INSERT INTO `Partimages` (`run_id`, `path`, `size_kb`, `x`, `y`, `w`, `h`, `created`, `imported`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
	cursor.execute(sql, (run_id, filename, size, int(x), int(y), int(w), int(h), created, imported))
	connection.commit()
	...
	for c in cnts:
			...
			if args.get("write", None) is None:
				...
				if cv2.contourArea(c) > minArea:
					...
					detectPartFromImageAndSave(partimage, x, y, w, h)
