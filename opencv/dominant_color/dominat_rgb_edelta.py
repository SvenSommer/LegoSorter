...
def getRGBforColorId(colorid):
    global connection, cursor
    sql = "SELECT color_code FROM LegoSorterDB.Colors WHERE color_id = " + str(colorid)
    cursor.execute(sql)
    color_code = cursor.fetchone()
    print("ColorCode for id = " + str(colorid) + " : " + color_code["color_code"])
    return hex2rgb(color_code["color_code"])

def loadimagepathsfromDB(runid):
    global connection, cursor
...
pathsandcolorids = loadimagepathsfromDB(8) 

for pathsandcolorid in pathsandcolorids:
    contour = segment(pathsandcolorid["path"])
    ...
    third = fig.add_subplot(Rows, Cells ,3)
    third.imshow(getPatchFromColor(getRGBforColorId(pathsandcolorid["color_id"]),contour ))
    third.set_title('Target: ' + pathsandcolorid["color_name"])
    third.axis('off')
    cposition= 6
