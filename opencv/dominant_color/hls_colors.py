...
# Connect to Database
    ...
    for color in colors:
        ...
        print(legocolor.colorname  + ", hex: " + str(legocolor.hex) + ", rgb "+ str(legocolor.rgb) + ",hsv " + str(legocolor.hsv)+ ",hls " + str(legocolor.hls)) 
...
imagecounter = 10
fig = plt.figure(1,figsize=(200, 60))
...
def main():
    ...
    dirpath = '/home/robert/LegoSorter/partimages/collection1/run4/'
    filenames = random.sample(os.listdir(dirpath), 3)
    for fname in filenames:
        srcpath = os.path.join(dirpath, fname)
        imagepaths.append(srcpath)

    plot = fig.add_subplot(Rows, Cells , imagecounter)

    Rows = len(imagepaths)

   
    for color in legocolors:
        plot.scatter(color.hls[0], color.hls[2], color=downscaleColorValues(color.rgb))

    for imagepath in imagepaths:
        image = cv2.imread(imagepath)
        drawImage(image,'Original')
        mean = segment(imagepath)
        plot.scatter(mean[0],mean[2], s=50, marker='x')

    plot.set_xlabel("hue")
    plot.set_ylabel("saturation")
